@extends('master.layout')

@section('title')
    Modifier echelle 
@endsection

@section('content')
    <div class="row my-4" align="center">
        <div class="col-md-8 mx-auto">
            @if ($errors->any())
                <div class="alert alert-danger">
                    <ul>
                        @foreach ($errors->all() as $error)
                            <li>{{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif
            <div class="card">
                <div class="class-header my-4">
                    <h3 align="center" class="class-title ">
                        Modifier un echelle 
                    </h3>   
                </div>
                <div class="card-body">
                    <form action="{{ route('echelle.update',$echelles->Ref_echelle) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">DRPP</label>
                            <input type="text" class="form-control" placeholder="Entrer le DRPP" 
                            value="{{ $echelles->DRPP }}"
                            name="DRPP">
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Desingation d'echelle</label>
                            <input type="text" class="form-control" placeholder="Entrer la designation" 
                            value="{{ $echelles->Designation_echelle }}"
                            name="Designation_echelle">
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Echellon</label>
                            <input type="text" class="form-control" placeholder="Entrer l'echellon" 
                            value="{{ $echelles->echellon }}"
                            name="echellon">
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Date d'echelle</label>
                            <input type="date" class="form-control" placeholder="" 
                            value="{{ $echelles->Date_echelle }}"
                            name="Date_echelle" readonly>
                        </div>
                        <div class="mb-3">
                            <button class="btn btn-primary">
                                Valider
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>        
    </div>
@endsection