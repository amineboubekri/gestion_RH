@extends('master.layout')

@section('title')
    Modifier mission 
@endsection

@section('content')
    <div class="row my-4" align="center">
        <div class="col-md-8 mx-auto">
            @if ($errors->any())
                <div class="alert alert-danger">
                    <ul>
                        @foreach ($errors->all() as $error)
                            <li>{{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif
            <div class="card">
                <div class="class-header my-4">
                    <h3 align="center" class="class-title ">
                        Modifier une mission 
                    </h3>   
                </div>
                <div class="card-body">
                    <form action="{{ route('mission.update',$missions->Ref_mission) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">DRPP</label>
                            <input type="text" class="form-control" placeholder="Entrer le DRPP" 
                            value="{{ $missions->DRPP }}"
                            name="DRPP" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Objet de mision</label>
                            <input type="text" class="form-control" placeholder="Entrer la designation" 
                            value="{{ $missions->Objet_mission }}"
                            name="Objet_mission">
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Ville de mission</label>
                            <input type="text" class="form-control" placeholder="Entrer la designation" 
                            value="{{ $missions->Ville_mission }}"
                            name="Ville_mission">
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Date de debut</label>
                            <input type="date" class="form-control" placeholder="" 
                            value="{{ $missions->Date_debut }}"
                            name="Date_debut">
                        </div>
                        <div class="mb-3">
                            <label for="exampleFormControlInput1" class="form-label">Date de retour</label>
                            <input type="date" class="form-control" placeholder="" 
                            value="{{ $missions->Date_retour }}"
                            name="Date_retour">
                        </div>
                        <div class="mb-3">
                            <button class="btn btn-primary">
                                Valider
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>        
    </div>
@endsection